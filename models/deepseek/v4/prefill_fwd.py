...
hidden state passes ``hc_head`` -> final ``rms_norm`` to produce normalized
hidden rows.  This is a kernel-only smoke driver: it does not run a golden
comparison.
"""
...
# final RMSNorm gamma, matching decode_fwd.
...
def build_tensor_specs(start_pos=0, num_tokens=T):
    ...
    specs.append(TensorSpec("x_out", [N_RANKS, T, D], torch.bfloat16, is_output=True))
